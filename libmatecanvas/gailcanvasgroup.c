...
gail_canvas_group_new (GObject *obj)
{
  ...
  g_return_val_if_fail (MATE_IS_CANVAS_GROUP (obj), NULL);
...
gail_canvas_group_get_n_children (AtkObject *obj)
{
  ...
  g_return_val_if_fail (MATE_IS_CANVAS_GROUP (g_obj), 0);
  group = MATE_CANVAS_GROUP (g_obj);
                             ...
                             gint      i)
{
  ...
  atk_gobject = ATK_GOBJECT_ACCESSIBLE (obj);
  g_obj = atk_gobject_accessible_get_object (atk_gobject);
  g_return_val_if_fail (MATE_IS_CANVAS_GROUP (g_obj), NULL);
  group = MATE_CANVAS_GROUP (g_obj);
  ...
  item = MATE_CANVAS_ITEM (list_item->data);
