...
#include "mate-canvas-rich-text.h"
#include "mate-canvas-i18n.h"

struct _MateCanvasRichTextPrivate {
...
static void mate_canvas_rich_text_class_init(MateCanvasRichTextClass *klass);
static void mate_canvas_rich_text_init(MateCanvasRichText *text);
static void mate_canvas_rich_text_set_property(GObject *object, guint property_id,
						const GValue *value, GParamSpec *pspec);
static void mate_canvas_rich_text_get_property(GObject *object, guint property_id,
						GValue *value, GParamSpec *pspec);
static void mate_canvas_rich_text_update(MateCanvasItem *item, double *affine,
...
static void mate_canvas_rich_text_ensure_layout(MateCanvasRichText *text);
static void mate_canvas_rich_text_destroy_layout(MateCanvasRichText *text);
static void mate_canvas_rich_text_start_cursor_blink(MateCanvasRichText *text, gboolean delay);
static void mate_canvas_rich_text_stop_cursor_blink(MateCanvasRichText *text);
static void mate_canvas_rich_text_move_cursor(MateCanvasRichText *text,
...
static GtkTextBuffer *get_buffer(MateCanvasRichText *text);
...
GType
mate_canvas_rich_text_get_type(void)
{
	static GType rich_text_type;

	if (!rich_text_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasRichTextClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_rich_text_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasRichText),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_rich_text_init,
			NULL			/* value_table */
		};

		rich_text_type = g_type_register_static (MATE_TYPE_CANVAS_ITEM, "MateCanvasRichText",
							 &object_info, 0);
	}
...
static void
mate_canvas_rich_text_finalize(GObject *object)
{
	MateCanvasRichText *text;

	text = MATE_CANVAS_RICH_TEXT(object);
...
mate_canvas_rich_text_class_init(MateCanvasRichTextClass *klass)
{
	...
	gobject_class->get_property = mate_canvas_rich_text_get_property;
	...
	signals[TAG_CHANGED] = g_signal_new(
		...
		G_SIGNAL_RUN_LAST,
		G_STRUCT_OFFSET(MateCanvasRichTextClass, tag_changed),
		NULL, NULL,
		g_cclosure_marshal_VOID__OBJECT,
		G_TYPE_NONE, 1,
		G_TYPE_OBJECT);

	gobject_class->finalize = mate_canvas_rich_text_finalize;

	item_class->update = mate_canvas_rich_text_update;
	item_class->realize = mate_canvas_rich_text_realize;
	item_class->unrealize = mate_canvas_rich_text_unrealize;
	item_class->draw = mate_canvas_rich_text_draw;
	item_class->point = mate_canvas_rich_text_point;
	item_class->render = mate_canvas_rich_text_render;
	item_class->event = mate_canvas_rich_text_event;
	item_class->bounds = mate_canvas_rich_text_get_bounds;
} /* mate_canvas_rich_text_class_init */

static void
mate_canvas_rich_text_init(MateCanvasRichText *text)
{
#if 0
	GtkObject *object = GTK_OBJECT(text);

	object->flags |= MATE_CANVAS_ITEM_ALWAYS_REDRAW;
#endif
	text->_priv = g_new0(MateCanvasRichTextPrivate, 1);
...
} /* mate_canvas_rich_text_init */

static void
mate_canvas_rich_text_set_property (GObject *object, guint property_id,
				     const GValue *value, GParamSpec *pspec)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(object);
	...
	case PROP_CURSOR_VISIBLE:
		...
		if (text->_priv->layout) {
			...
			if (text->_priv->cursor_visible && text->_priv->cursor_blink) {
				mate_canvas_rich_text_start_cursor_blink(
					text, FALSE);
			}
			else
				mate_canvas_rich_text_stop_cursor_blink(text);
		}
		break;
	...
	mate_canvas_item_request_update(MATE_CANVAS_ITEM(text));
}

static void
mate_canvas_rich_text_get_property (GObject *object, guint property_id,
				     GValue *value, GParamSpec *pspec)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(object);
...
static void
mate_canvas_rich_text_realize(MateCanvasItem *item)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);

	(* MATE_CANVAS_ITEM_CLASS(parent_class)->realize)(item);

	mate_canvas_rich_text_ensure_layout(text);
} /* mate_canvas_rich_text_realize */

static void
mate_canvas_rich_text_unrealize(MateCanvasItem *item)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);

	mate_canvas_rich_text_destroy_layout(text);

	(* MATE_CANVAS_ITEM_CLASS(parent_class)->unrealize)(item);
} /* mate_canvas_rich_text_unrealize */

static void
mate_canvas_rich_text_move_iter_by_lines(MateCanvasRichText *text,
...
static gint
mate_canvas_rich_text_get_cursor_x_position(MateCanvasRichText *text)
{
	GtkTextIter insert;
	GdkRectangle rect;
	...
	return rect.x;
} /* mate_canvas_rich_text_get_cursor_x_position */

static void
mate_canvas_rich_text_move_cursor(MateCanvasRichText *text,
				   GtkMovementStep step,
...
static void
mate_canvas_rich_text_delete_from_cursor(MateCanvasRichText *text,
					  GtkDeleteType type,
					  gint count)
{
...
} /* mate_canvas_rich_text_delete_from_cursor */

static gint
selection_motion_event_handler(MateCanvasRichText *text, GdkEvent *event,
			       gpointer data)
{
...
static void
mate_canvas_rich_text_start_selection_drag(MateCanvasRichText *text,
					    ...
					    GdkEventButton *button)
{
...
#if 0
	mate_canvas_item_grab_focus(MATE_CANVAS_ITEM(text));
#endif
	...
	text->_priv->selection_drag_handler = g_signal_connect(
		...
		NULL);
} /* mate_canvas_rich_text_start_selection_drag */

static gboolean
mate_canvas_rich_text_end_selection_drag(MateCanvasRichText *text,
...
#if 0
	mate_canvas_item_grab(NULL);
#endif

	return TRUE;
} /* mate_canvas_rich_text_end_selection_drag */

static void
mate_canvas_rich_text_emit_tag_changed(MateCanvasRichText *text,
					GtkTextTag *tag)
{
...
{
	return FALSE;
} /* mate_canvas_rich_text_key_release_event */
...
static gint
mate_canvas_rich_text_button_press_event(MateCanvasItem *item,
					  GdkEventButton *event)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	GtkTextIter iter;
...
static gint
mate_canvas_rich_text_button_release_event(MateCanvasItem *item,
					    GdkEventButton *event)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	double newx, newy;
	...
	if (event->button == 1) {
		...
		if (mate_canvas_rich_text_end_selection_drag(text, event))
			return TRUE;
	...
	return FALSE;
} /* mate_canvas_rich_text_button_release_event */

static gint
mate_canvas_rich_text_focus_in_event(MateCanvasItem *item,
				      GdkEventFocus *event)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	...
	if (text->_priv->cursor_visible && text->_priv->layout) {
		...
		mate_canvas_rich_text_start_cursor_blink(text, FALSE);
	}

	return FALSE;
} /* mate_canvas_rich_text_focus_in_event */

static gint
mate_canvas_rich_text_focus_out_event(MateCanvasItem *item,
				       GdkEventFocus *event)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	...
	return FALSE;
} /* mate_canvas_rich_text_focus_out_event */
...
static gint
mate_canvas_rich_text_event(MateCanvasItem *item, GdkEvent *event)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	int x, y;
	...
	switch (event->type) {
	case GDK_KEY_PRESS:
		return mate_canvas_rich_text_key_press_event(
			item, (GdkEventKey *) event);
	case GDK_KEY_RELEASE:
		return mate_canvas_rich_text_key_release_event(
			item, (GdkEventKey *) event);
	case GDK_BUTTON_PRESS:
		return mate_canvas_rich_text_button_press_event(
			item, (GdkEventButton *) event);
	case GDK_BUTTON_RELEASE:
		return mate_canvas_rich_text_button_release_event(
			item, (GdkEventButton *) event);
	case GDK_FOCUS_CHANGE:
		...
		if (((GdkEventFocus *) event)->in)
			return mate_canvas_rich_text_focus_in_event(
				item, (GdkEventFocus *) event);
		else
			return mate_canvas_rich_text_focus_out_event(
				item, (GdkEventFocus *) event);
	default:
		return FALSE;
	}
} /* mate_canvas_rich_text_event */
 ...
 **/
void
mate_canvas_rich_text_cut_clipboard(MateCanvasRichText *text)
{
...
/**
 * mate_canvas_rich_text_copy_clipboard:
 * @text: a #MateCanvasRichText.
 *
 * Copies the currently selected @text to clipboard.
 **/
void
mate_canvas_rich_text_copy_clipboard(MateCanvasRichText *text)
{
	g_return_if_fail(text);
	g_return_if_fail(get_buffer(text));
...
/**
 * mate_canvas_rich_text_paste_clipboard:
 * @text: a #MateCanvasRichText.
 *
 * Pastes the contents of the clipboard at the insertion point.
 **/
void
mate_canvas_rich_text_paste_clipboard(MateCanvasRichText *text)
{
	g_return_if_fail(text);
	g_return_if_fail(get_buffer(text));
...
static void
mate_canvas_rich_text_start_cursor_blink(MateCanvasRichText *text,
					  gboolean with_delay)
{
	if (!text->_priv->layout)
		return;
...
static void
mate_canvas_rich_text_stop_cursor_blink(MateCanvasRichText *text)
{
	if (text->_priv->blink_timeout) {
		g_source_remove(text->_priv->blink_timeout);
		text->_priv->blink_timeout = 0;
	}
} /* mate_canvas_rich_text_stop_cursor_blink */
...
/**
 * mate_canvas_rich_text_set_buffer:
 * @text: a #MateCanvasRichText.
...
get_buffer(MateCanvasRichText *text)
{
	if (!text->_priv->buffer) {
		GtkTextBuffer *b;

		b = gtk_text_buffer_new(NULL);
		mate_canvas_rich_text_set_buffer(text, b);
...
/**
 * mate_canvas_rich_text_get_buffer:
 * @text: a #MateCanvasRichText.
 *
 * Returns a #GtkTextBuffer associated with the #MateCanvasRichText.
 * This function creates a new #GtkTextBuffer if the text buffer is NULL.
 *
 * Return value: the #GtkTextBuffer.
 **/
GtkTextBuffer *
mate_canvas_rich_text_get_buffer(MateCanvasRichText *text)
{
	g_return_val_if_fail(MATE_IS_CANVAS_RICH_TEXT(text), NULL);
...
/**
 * mate_canvas_rich_text_get_iter_location:
 * @text: a #MateCanvasRichText.
 * @iter: a #GtkTextIter.
...
/**
 * mate_canvas_rich_text_get_iter_at_location:
 * @text: a #MateCanvasRichText.
 * @iter: a #GtkTextIter.
...
static void
mate_canvas_rich_text_set_attributes_from_style(MateCanvasRichText *text,
...
} /* mate_canvas_rich_text_set_attributes_from_style */

static void
mate_canvas_rich_text_ensure_layout(MateCanvasRichText *text)
{
	if (!text->_priv->layout) {
		GtkWidget *canvas;
		...
		canvas = GTK_WIDGET(MATE_CANVAS_ITEM(text)->canvas);
...
} /* mate_canvas_rich_text_ensure_layout */

static void
mate_canvas_rich_text_destroy_layout(MateCanvasRichText *text)
{
	if (text->_priv->layout) {
...
} /* mate_canvas_rich_text_destroy_layout */

static void
adjust_for_anchors(MateCanvasRichText *text, double *ax, double *ay)
{
	double x, y;
...
static void
get_bounds(MateCanvasRichText *text, double *px1, double *py1,
	   double *px2, double *py2)
{
	MateCanvasItem *item = MATE_CANVAS_ITEM(text);
	double x, y;
	double x1, x2, y1, y2;
	int cx1, cx2, cy1, cy2;
	...
	mate_canvas_item_i2w(item, &x2, &y2);
	mate_canvas_w2c(item->canvas, x1, y1, &cx1, &cy1);
	mate_canvas_w2c(item->canvas, x2, y2, &cx2, &cy2);
...
static void mate_canvas_rich_text_get_bounds(MateCanvasItem *item, double *px1, double *py1,
	   double *px2, double *py2)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
...
static void
mate_canvas_rich_text_update(MateCanvasItem *item, double *affine,
			      ArtSVP *clip_path, int flags)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	double x1, y1, x2, y2;
	...
	(* MATE_CANVAS_ITEM_CLASS(parent_class)->update)(
...
} /* mate_canvas_rich_text_point */

static void
mate_canvas_rich_text_draw(MateCanvasItem *item, GdkDrawable *drawable,
			    int x, int y, int width, int height)
{
	MateCanvasRichText *text = MATE_CANVAS_RICH_TEXT(item);
	...
	gtk_text_layout_draw(
		...
		NULL);
} /* mate_canvas_rich_text_draw */

static void
mate_canvas_rich_text_render(MateCanvasItem *item, MateCanvasBuf *buf)
{
	g_warning ("rich text item not implemented for anti-aliased canvas");
} /* mate_canvas_rich_text_render */
