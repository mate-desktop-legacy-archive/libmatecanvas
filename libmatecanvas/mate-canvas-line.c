...
static void mate_canvas_line_class_init   (MateCanvasLineClass *class);
static void mate_canvas_line_init         (MateCanvasLine      *line);
static void mate_canvas_line_destroy      (GtkObject            *object);
static void mate_canvas_line_set_property (GObject              *object,
					    guint                 param_id,
					    const GValue         *value,
					    GParamSpec           *pspec);
static void mate_canvas_line_get_property (GObject              *object,
					    guint                 param_id,
					    GValue               *value,
					    GParamSpec           *pspec);

static void   mate_canvas_line_update      (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags);
static void   mate_canvas_line_realize     (MateCanvasItem *item);
static void   mate_canvas_line_unrealize   (MateCanvasItem *item);
static void   mate_canvas_line_draw        (MateCanvasItem *item, GdkDrawable *drawable,
					     int x, int y, int width, int height);
static double mate_canvas_line_point       (MateCanvasItem *item, double x, double y,
					     int cx, int cy, MateCanvasItem **actual_item);
static void   mate_canvas_line_bounds      (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2);
static void   mate_canvas_line_render      (MateCanvasItem *item, MateCanvasBuf *buf);
...
GType
mate_canvas_line_get_type (void)
{
	static GType line_type;

	if (!line_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasLineClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_line_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasLine),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_line_init,
			NULL			/* value_table */
		};

		line_type = g_type_register_static (MATE_TYPE_CANVAS_ITEM, "MateCanvasLine",
						    &object_info, 0);
	}
...
static void
mate_canvas_line_class_init (MateCanvasLineClass *class)
{
	...
	gobject_class->set_property = mate_canvas_line_set_property;
	gobject_class->get_property = mate_canvas_line_get_property;
	...
	object_class->destroy = mate_canvas_line_destroy;

	item_class->update = mate_canvas_line_update;
	item_class->realize = mate_canvas_line_realize;
	item_class->unrealize = mate_canvas_line_unrealize;
	item_class->draw = mate_canvas_line_draw;
	item_class->point = mate_canvas_line_point;
	item_class->bounds = mate_canvas_line_bounds;

	item_class->render = mate_canvas_line_render;
}

static void
mate_canvas_line_init (MateCanvasLine *line)
{
	line->width = 0.0;
...
static void
mate_canvas_line_destroy (GtkObject *object)
{
	MateCanvasLine *line;

	g_return_if_fail (object != NULL);
	g_return_if_fail (MATE_IS_CANVAS_LINE (object));

	line = MATE_CANVAS_LINE (object);
 ...
 */
static void
get_bounds (MateCanvasLine *line, double *bx1, double *by1, double *bx2, double *by2)
{
	double *coords;
	double x1, y1, x2, y2;
	double width;
	...
	if (line->join == GDK_JOIN_MITER)
		for (i = line->num_points, coords = line->coords; i >= 3; i--, coords += 2) {
			...
			if (mate_canvas_get_miter_points (coords[0], coords[1],
							   coords[2], coords[3],
							   coords[4], coords[5],
							   width,
							   &mx1, &my1, &mx2, &my2)) {
 ...
 */
static void
get_bounds_canvas (MateCanvasLine *line, double *bx1, double *by1, double *bx2, double *by2, double affine[6])
{
...
static void
reconfigure_arrows (MateCanvasLine *line)
{
	double *poly, *coords;
	double dx, dy, length;
...
static void
set_line_gc_foreground (MateCanvasLine *line)
{
	GdkColor c;
...
static void
set_line_gc_width (MateCanvasLine *line)
{
	int width;
...
static void
set_stipple (MateCanvasLine *line, GdkBitmap *stipple, int reconfigure)
{
...
static void
mate_canvas_line_set_property (GObject              *object,
				guint                 param_id,
				const GValue         *value,
				GParamSpec           *pspec)
{
	...
	case PROP_FILL_STIPPLE:
		...
		mate_canvas_item_request_redraw_svp (item, line->fill_svp);
		break;
	...
	if (color_changed) {
		if (have_pixel)
			...
			line->fill_pixel = mate_canvas_get_color_pixel (item->canvas,
									 line->fill_rgba);
		...
		if (line->first_svp) 
			mate_canvas_item_request_redraw_svp (item, line->first_svp);

		if (line->last_svp) 
			mate_canvas_item_request_redraw_svp (item, line->last_svp);
 ...
 * arrowheads.
 */
static MateCanvasPoints *
get_points (MateCanvasLine *line)
{
	MateCanvasPoints *points;
	...
	points = mate_canvas_points_new (line->num_points);
...
static void
mate_canvas_line_get_property (GObject              *object,
				guint                 param_id,
				GValue               *value,
				GParamSpec           *pspec)
{
	...
	case PROP_FILL_COLOR_GDK: {
		MateCanvas *canvas = MATE_CANVAS_ITEM (line)->canvas;
...
static void
mate_canvas_line_render (MateCanvasItem *item,
			  MateCanvasBuf *buf)
{
	MateCanvasLine *line;

	line = MATE_CANVAS_LINE (item);

	if (line->fill_svp != NULL)
		mate_canvas_render_svp (buf, line->fill_svp, line->fill_rgba);

	if (line->first_svp != NULL)
		mate_canvas_render_svp (buf, line->first_svp, line->fill_rgba);

	if (line->last_svp != NULL)
		mate_canvas_render_svp (buf, line->last_svp, line->fill_rgba);
}
...
static void
mate_canvas_line_update (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	MateCanvasLine *line;
	int i;
	ArtVpath *vpath;
	...
	if (item->canvas->aa) {
		mate_canvas_item_reset_bounds (item);
		...
		svp = art_svp_vpath_stroke (vpath,
					    mate_canvas_join_gdk_to_art (line->join),
					    mate_canvas_cap_gdk_to_art (line->cap),
					    width,
					    4,
					    0.25);
		art_free (vpath);

		mate_canvas_item_update_svp_clip (item, &line->fill_svp, svp, clip_path);

		if (line->first_arrow && line->first_coords) {
			svp = svp_from_points (line->first_coords, NUM_ARROW_POINTS, affine);
                        mate_canvas_item_update_svp_clip (item, 
...
static void
mate_canvas_line_realize (MateCanvasItem *item)
{
	MateCanvasLine *line;
...
#if 0
	(* MATE_CANVAS_ITEM_CLASS (item->object.klass)->update) (item, NULL, NULL, 0);
#endif
}

static void
mate_canvas_line_unrealize (MateCanvasItem *item)
{
	MateCanvasLine *line;
...
static void
item_to_canvas (MateCanvas *canvas, double *item_coords, GdkPoint *canvas_coords, int num_points,
...
static void
mate_canvas_line_draw (MateCanvasItem *item, GdkDrawable *drawable,
			int x, int y, int width, int height)
{
	MateCanvasLine *line;
...
static double
mate_canvas_line_point (MateCanvasItem *item, double x, double y,
			 int cx, int cy, MateCanvasItem **actual_item)
{
	MateCanvasLine *line;
...
#ifdef VERBOSE
	g_print ("mate_canvas_line_point x, y = (%g, %g); cx, cy = (%d, %d)\n", x, y, cx, cy);
#endif
	...
	*actual_item = item;
	...
	for (i = num_points, coords = line_points; i >= 2; i--, coords += 2) {
		...
		if (i == num_points)
			mate_canvas_get_butt_points (coords[2], coords[3], coords[0], coords[1],
			...
			if ((line->join == GDK_JOIN_BEVEL) || changed_miter_to_bevel) {
				...
				dist = mate_canvas_polygon_to_point (poly, 5, x, y);
				if (dist < MATE_CANVAS_EPSILON) {
					best = 0.0;
					goto done;
				} else if (dist < best)
		...
		if (i == 2)
			mate_canvas_get_butt_points (coords[0], coords[1], coords[2], coords[3],
						      width, (line->cap == GDK_CAP_PROJECTING),
	...
	/* sometimes the MateCanvasItem::update signal will not have
	...
	if (line->first_arrow) {
		dist = mate_canvas_polygon_to_point (line->first_coords, NUM_ARROW_POINTS, x, y);
		if (dist < MATE_CANVAS_EPSILON) {
			best = 0.0;
			goto done;
		} else
	...
	if (line->last_arrow) {
		dist = mate_canvas_polygon_to_point (line->last_coords, NUM_ARROW_POINTS, x, y);
		if (dist < MATE_CANVAS_EPSILON) {
			best = 0.0;
			goto done;
		} else
...
static void
mate_canvas_line_bounds (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	MateCanvasLine *line;
