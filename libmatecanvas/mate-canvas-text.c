...
/*
  @NOTATION@
 */
/* Text item type for MateCanvas widget
 *
...
struct _MateCanvasTextPrivate {
	guint render_dirty : 1;
	FT_Bitmap bitmap;
};


static void mate_canvas_text_class_init (MateCanvasTextClass *class);
static void mate_canvas_text_init (MateCanvasText *text);
static void mate_canvas_text_destroy (GtkObject *object);
static void mate_canvas_text_set_property (GObject            *object,
					    guint               param_id,
					    const GValue       *value,
					    GParamSpec         *pspec);
static void mate_canvas_text_get_property (GObject            *object,
					    guint               param_id,
					    GValue             *value,
					    GParamSpec         *pspec);

static void mate_canvas_text_update (MateCanvasItem *item, double *affine,
				      ArtSVP *clip_path, int flags);
static void mate_canvas_text_realize (MateCanvasItem *item);
static void mate_canvas_text_unrealize (MateCanvasItem *item);
static void mate_canvas_text_draw (MateCanvasItem *item, GdkDrawable *drawable,
				    int x, int y, int width, int height);
static double mate_canvas_text_point (MateCanvasItem *item, double x, double y, int cx, int cy,
				       MateCanvasItem **actual_item);
static void mate_canvas_text_bounds (MateCanvasItem *item,
				      double *x1, double *y1, double *x2, double *y2);
static void mate_canvas_text_render (MateCanvasItem *item, MateCanvasBuf *buf);

static void mate_canvas_text_set_markup (MateCanvasText *textitem,
					  const gchar     *markup);

static void mate_canvas_text_set_font_desc    (MateCanvasText *textitem,
					        PangoFontDescription *font_desc);

static void mate_canvas_text_apply_font_desc  (MateCanvasText *textitem);
static void mate_canvas_text_apply_attributes (MateCanvasText *textitem);
...
/**
 * mate_canvas_text_get_type:
 * @void: 
 * 
 * Registers the &MateCanvasText class if necessary, and returns the type ID
 * associated to it.
 * 
 * Return value: The type ID of the &MateCanvasText class.
 **/
GType
mate_canvas_text_get_type (void)
{
	static GType text_type;

	if (!text_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasTextClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_text_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasText),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_text_init,
			NULL			/* value_table */
		};

		text_type = g_type_register_static (MATE_TYPE_CANVAS_ITEM, "MateCanvasText",
						    &object_info, 0);
	}
...
static void
mate_canvas_text_class_init (MateCanvasTextClass *class)
{
	...
	gobject_class->set_property = mate_canvas_text_set_property;
	gobject_class->get_property = mate_canvas_text_get_property;
...
#undef ADD_SET_PROP
	...
	object_class->destroy = mate_canvas_text_destroy;

	item_class->update = mate_canvas_text_update;
	item_class->realize = mate_canvas_text_realize;
	item_class->unrealize = mate_canvas_text_unrealize;
	item_class->draw = mate_canvas_text_draw;
	item_class->point = mate_canvas_text_point;
	item_class->bounds = mate_canvas_text_bounds;
	item_class->render = mate_canvas_text_render;
}

/* Object initialization function for the text item */
static void
mate_canvas_text_init (MateCanvasText *text)
{
	text->x = 0.0;
	text->y = 0.0;
	...
	text->priv = g_new (MateCanvasTextPrivate, 1);
...
static void
mate_canvas_text_destroy (GtkObject *object)
{
	MateCanvasText *text;

	g_return_if_fail (MATE_IS_CANVAS_TEXT (object));

	text = MATE_CANVAS_TEXT (object);
...
static void
get_bounds (MateCanvasText *text, double *px1, double *py1, double *px2, double *py2)
{
	MateCanvasItem *item;
	double wx, wy;

	item = MATE_CANVAS_ITEM (text);
	...
	mate_canvas_w2c (item->canvas, wx + text->xofs, wy + text->yofs, &text->cx, &text->cy);

	/* Get canvas pixel coordinates for clip rectangle position */

	mate_canvas_w2c (item->canvas, wx, wy, &text->clip_cx, &text->clip_cy);
...
static void
set_text_gc_foreground (MateCanvasText *text)
{
	GdkColor c;
...
static void
set_stipple (MateCanvasText *text, GdkBitmap *stipple, int reconfigure)
{
...
static void
ensure_font (MateCanvasText *text)
{
	if (!text->font_desc)
...
static void
mate_canvas_text_set_property (GObject            *object,
				guint               param_id,
				const GValue       *value,
				GParamSpec         *pspec)
{
	...
	case PROP_MARKUP:
		mate_canvas_text_set_markup (text,
	...
	case PROP_FONT: {
		...
		mate_canvas_text_set_font_desc (text, font_desc);
		if (font_desc)
	...
	case PROP_FONT_DESC:
		mate_canvas_text_set_font_desc (text, g_value_peek_pointer (value));
		break;
	...
	case PROP_SIZE_POINTS:
		...
		mate_canvas_text_apply_font_desc (text);
		text->priv->render_dirty = 1;
		break;
	...
	case PROP_UNDERLINE:
		...
		mate_canvas_text_apply_attributes (text);
		text->priv->render_dirty = 1;
		break;
	...
	if (color_changed) {
		if (have_pixel)
			...
			text->pixel = mate_canvas_get_color_pixel (item->canvas, text->rgba);
...
static void
mate_canvas_text_get_property (GObject            *object,
				guint               param_id,
				GValue             *value,
				GParamSpec         *pspec)
{
	...
	case PROP_FILL_COLOR_GDK: {
		MateCanvas *canvas = MATE_CANVAS_ITEM (text)->canvas;
...
/* */
static void
mate_canvas_text_apply_font_desc (MateCanvasText *text)
{
	PangoFontDescription *font_desc =
		pango_font_description_copy (
			GTK_WIDGET (MATE_CANVAS_ITEM (text)->canvas)->style->font_desc);
...
/* */
static void
mate_canvas_text_apply_attributes (MateCanvasText *text)
{
...
static void
mate_canvas_text_set_font_desc (MateCanvasText      *text,
				 PangoFontDescription *font_desc)
{
	if (text->font_desc)
		pango_font_description_free (text->font_desc);
...
static void
mate_canvas_text_set_markup (MateCanvasText *textitem,
			      const gchar     *markup)
{
	...
	mate_canvas_text_apply_attributes (textitem);
}

/* Update handler for the text item */
static void
mate_canvas_text_update (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	MateCanvasText *text;
	double x1, y1, x2, y2;

	text = MATE_CANVAS_TEXT (item);
...
static void
mate_canvas_text_realize (MateCanvasItem *item)
{
	MateCanvasText *text;
...
static void
mate_canvas_text_unrealize (MateCanvasItem *item)
{
	MateCanvasText *text;
...
static void
mate_canvas_text_draw (MateCanvasItem *item, GdkDrawable *drawable,
			int x, int y, int width, int height)
{
	MateCanvasText *text;
	GdkRectangle rect;
	...
	if (text->stipple)
		mate_canvas_set_stipple_origin (item->canvas, text->gc);
...
static void
mate_canvas_text_render (MateCanvasItem *item, MateCanvasBuf *buf)
{
	MateCanvasText *text;
	guint32 fg_color;
...
static double
mate_canvas_text_point (MateCanvasItem *item, double x, double y,
...
static void
mate_canvas_text_bounds (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	MateCanvasText *text;
	double width, height;
