...
static void       gail_canvas_item_remove_focus_handler     (AtkComponent      *component,
					                     guint             handler_id);
static gboolean   is_item_on_screen                         (MateCanvasItem   *item);
static void       get_item_extents                          (MateCanvasItem   *item,
					                     gint              *x,
					                     gint              *y,
					                     gint              *width,
					                     gint              *height);
static gboolean   is_item_in_window                         (MateCanvasItem   *item,
					                     gint              x,
					                     gint              y,
					                     gint              width,
					                     gint              height);
...
gail_canvas_item_new (GObject *obj)
{
  ...
  g_return_val_if_fail (MATE_IS_CANVAS_ITEM (obj), NULL);
...
gail_canvas_item_get_index_in_parent (AtkObject *obj)
{
  ...
  if (item->parent)
    {
      return g_list_index (MATE_CANVAS_GROUP (item->parent)->item_list, item);
    }
  else
    {
                              ...
                              AtkCoordType coord_type)
{
  ...
  /* Get the MateCanvasItem */
  item = MATE_CANVAS_ITEM (obj);
...
gail_canvas_item_grab_focus (AtkComponent    *component)
{
  ...
  mate_canvas_item_grab_focus (item);
...
static gboolean 
is_item_on_screen (MateCanvasItem *item)
{
...
static void
get_item_extents (MateCanvasItem   *item,
                  gint              *x,
                  gint              *y,
                  gint              *width,
                  gint              *height)
{
  ...
  if (MATE_CANVAS_ITEM_CLASS (G_OBJECT_GET_CLASS (item))->bounds)
    (* MATE_CANVAS_ITEM_CLASS (G_OBJECT_GET_CLASS (item))->bounds) (item, &bx1, &by1, &bx2, &by2);
  ...
  mate_canvas_get_scroll_offsets (item->canvas, &scroll_x, &scroll_y);
...
static gboolean
is_item_in_window (MateCanvasItem   *item,
                   gint              x,
                   gint              y,
                   gint              width,
                   gint              height)
