 ...
 *
 * - Allow to specify whether MateCanvasImage sizes are in units or pixels (scale or don't scale).
 *
 * - Implement a flag for mate_canvas_item_reparent() that tells the function to keep the item
 ...
 *
 * - GList *mate_canvas_gimme_all_items_contained_in_this_area (MateCanvas *canvas, Rectangle area);
 *
 * - Retrofit all the primitive items with microtile support.
 *
 * - Curve support for line item.
 *
 * - Arc item (Havoc has it; to be integrated in MateCanvasEllipse).
 *
...
static void mate_canvas_request_update (MateCanvas      *canvas);
static void group_add                   (MateCanvasGroup *group,
					 MateCanvasItem  *item);
static void group_remove                (MateCanvasGroup *group,
					 MateCanvasItem  *item);
static void add_idle                    (MateCanvas      *canvas);


/*** MateCanvasItem ***/

/* Some convenience stuff */
#define GCI_UPDATE_MASK (MATE_CANVAS_UPDATE_REQUESTED | MATE_CANVAS_UPDATE_AFFINE | MATE_CANVAS_UPDATE_CLIP | MATE_CANVAS_UPDATE_VISIBILITY)
...
static void mate_canvas_item_class_init     (MateCanvasItemClass *class);
static void mate_canvas_item_init           (MateCanvasItem      *item);
static int  emit_event                       (MateCanvas *canvas, GdkEvent *event);
 ...
 *
 * Registers the &MateCanvasItem class if necessary, and returns the type ID
 * associated to it.
 *
 * Return value:  The type ID of the &MateCanvasItem class.
 **/
GType
mate_canvas_item_get_type (void)
{
	static GType canvas_item_type;

	if (!canvas_item_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasItemClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_item_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasItem),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_item_init,
			NULL			/* value_table */
		};

		canvas_item_type = g_type_register_static (GTK_TYPE_OBJECT, "MateCanvasItem",
							   &object_info, 0);
	}
...
/* Object initialization function for MateCanvasItem */
static void
mate_canvas_item_init (MateCanvasItem *item)
{
	item->object.flags |= MATE_CANVAS_ITEM_VISIBLE;
}

/**
 * mate_canvas_item_new:
 ...
 * mate_canvas_rect_get_type().  The list of object arguments/value pairs is
 ...
 * mate_canvas_item_construct() to set up the canvas item.
 *
 * Return value: The newly-created item.
 **/
MateCanvasItem *
mate_canvas_item_new (MateCanvasGroup *parent, GType type, const gchar *first_arg_name, ...)
{
	MateCanvasItem *item;
	va_list args;

	g_return_val_if_fail (MATE_IS_CANVAS_GROUP (parent), NULL);
	g_return_val_if_fail (g_type_is_a (type, mate_canvas_item_get_type ()), NULL);

	item = MATE_CANVAS_ITEM (g_object_new (type, NULL));

	va_start (args, first_arg_name);
	mate_canvas_item_construct (item, parent, first_arg_name, args);
	va_end (args);
 ...
 */
static void
item_post_create_setup (MateCanvasItem *item)
{
	group_add (MATE_CANVAS_GROUP (item->parent), item);

	mate_canvas_request_redraw (item->canvas, item->x1, item->y1, item->x2 + 1, item->y2 + 1);
	item->canvas->need_repick = TRUE;
}

/* Set_property handler for canvas items */
static void
mate_canvas_item_set_property (GObject *gobject, guint param_id,
				const GValue *value, GParamSpec *pspec)
{
	MateCanvasItem *item;

	g_return_if_fail (MATE_IS_CANVAS_ITEM (gobject));

	item = MATE_CANVAS_ITEM (gobject);
	...
	case ITEM_PROP_PARENT:
		if (item->parent != NULL) {
			...
			item->parent = MATE_CANVAS_ITEM (g_value_get_object (value));
...
static void
mate_canvas_item_get_property (GObject *gobject, guint param_id,
 ...
 **/
void
mate_canvas_item_construct (MateCanvasItem *item, MateCanvasGroup *parent,
			     const gchar *first_arg_name, va_list args)
{
	g_return_if_fail (MATE_IS_CANVAS_GROUP (parent));
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));

	item->parent = MATE_CANVAS_ITEM (parent);
	item->canvas = item->parent->canvas;
...
static void
redraw_if_visible (MateCanvasItem *item)
{
...
static void
mate_canvas_item_dispose (GObject *object)
{
	MateCanvasItem *item;

	g_return_if_fail (MATE_IS_CANVAS_ITEM (object));
	...
	if (item->object.flags & MATE_CANVAS_ITEM_MAPPED)
		(* MATE_CANVAS_ITEM_GET_CLASS (item)->unmap) (item);

	if (item->object.flags & MATE_CANVAS_ITEM_REALIZED)
		(* MATE_CANVAS_ITEM_GET_CLASS (item)->unrealize) (item);

	if (item->parent)
		group_remove (MATE_CANVAS_GROUP (item->parent), item);
...
static void
mate_canvas_item_realize (MateCanvasItem *item)
{
	GTK_OBJECT_SET_FLAGS (item, MATE_CANVAS_ITEM_REALIZED);
...
static void
mate_canvas_item_unrealize (MateCanvasItem *item)
{
	GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_REALIZED);
}

/* Map handler for canvas items */
static void
mate_canvas_item_map (MateCanvasItem *item)
{
	GTK_OBJECT_SET_FLAGS (item, MATE_CANVAS_ITEM_MAPPED);
}

/* Unmap handler for canvas items */
static void
mate_canvas_item_unmap (MateCanvasItem *item)
{
	GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_MAPPED);
}

/* Update handler for canvas items */
static void
mate_canvas_item_update (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_NEED_UPDATE);
	GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_NEED_AFFINE);
	GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_NEED_CLIP);
	GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_NEED_VIS);
}
...
static void
mate_canvas_item_invoke_update (MateCanvasItem *item, double *p2cpx, ArtSVP *clip_path, int flags)
{
	int child_flags;
	gdouble i2cpx[6];
	...
	if (item->xform) {
		if (item->object.flags & MATE_CANVAS_ITEM_AFFINE_FULL) {
	...
	if (child_flags & GCI_UPDATE_MASK) {
		if (MATE_CANVAS_ITEM_GET_CLASS (item)->update)
			MATE_CANVAS_ITEM_GET_CLASS (item)->update (item, i2cpx, clip_path, child_flags);
	}
}
...
static double
mate_canvas_item_invoke_point (MateCanvasItem *item, double x, double y, int cx, int cy, MateCanvasItem **actual_item)
{
	...
	if (MATE_CANVAS_ITEM_GET_CLASS (item)->point)
		return MATE_CANVAS_ITEM_GET_CLASS (item)->point (item, x, y, cx, cy, actual_item);

	return 1e18;
}

/**
 * mate_canvas_item_set:
 * @item: A canvas item.
 ...
 **/
void
mate_canvas_item_set (MateCanvasItem *item, const gchar *first_arg_name, ...)
{
	va_list args;

	va_start (args, first_arg_name);
	mate_canvas_item_set_valist (item, first_arg_name, args);
	va_end (args);
}


/**
 * mate_canvas_item_set_valist:
 * @item: A canvas item.
 ...
 **/
void
mate_canvas_item_set_valist (MateCanvasItem *item, const gchar *first_arg_name, va_list args)
{
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
...
/**
 * mate_canvas_item_affine_relative:
 * @item: A canvas item.
...
void
mate_canvas_item_affine_relative (MateCanvasItem *item, const double affine[6])
{
	gdouble i2p[6];
	...
	mate_canvas_item_affine_absolute (item, i2p);
}

/**
 * mate_canvas_item_affine_absolute:
 * @item: A canvas item.
 * @affine: An affine transformation matrix.
 *
 ...
 **/
void
mate_canvas_item_affine_absolute (MateCanvasItem *item, const double i2p[6])
{
	g_return_if_fail (item != NULL);
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
	...
	if (i2p) {
		if (item->xform && !(item->object.flags & MATE_CANVAS_ITEM_AFFINE_FULL)) {
		...
		item->object.flags |= MATE_CANVAS_ITEM_AFFINE_FULL;
	} else {
		if (item->xform) {
			g_free (item->xform);
			item->xform = NULL;
	...
	if (!(item->object.flags & MATE_CANVAS_ITEM_NEED_AFFINE)) {
		/* Request update */
		item->object.flags |= MATE_CANVAS_ITEM_NEED_AFFINE;
		mate_canvas_item_request_update (item);
	}
 ...
 **/
void
mate_canvas_item_move (MateCanvasItem *item, double dx, double dy)
{
	double translate[6];
	...
	mate_canvas_item_affine_relative (item, translate);
}
...
{
	MateCanvasGroup *parent;
	GList *old_before, *old_after;
	GList *after;

	parent = MATE_CANVAS_GROUP (MATE_CANVAS_ITEM (link->data)->parent);
 ...
 **/
void
mate_canvas_item_raise (MateCanvasItem *item, int positions)
{
	GList *link, *before;
	MateCanvasGroup *parent;
	...
	parent = MATE_CANVAS_GROUP (item->parent);
 ...
 **/
void
mate_canvas_item_lower (MateCanvasItem *item, int positions)
{
	GList *link, *before;
	MateCanvasGroup *parent;
...
/**
 * mate_canvas_item_raise_to_top:
 * @item: A canvas item.
 *
 * Raises an item to the top of its parent's stack.
 **/
void
mate_canvas_item_raise_to_top (MateCanvasItem *item)
{
	GList *link;
	MateCanvasGroup *parent;
...
/**
 * mate_canvas_item_lower_to_bottom:
 * @item: A canvas item.
 *
 * Lowers an item to the bottom of its parent's stack.
 **/
void
mate_canvas_item_lower_to_bottom (MateCanvasItem *item)
{
	GList *link;
	MateCanvasGroup *parent;
...
/**
 * mate_canvas_item_show:
 * @item: A canvas item.
 *
 * Shows a canvas item.  If the item was already shown, then no action is taken.
 **/
void
mate_canvas_item_show (MateCanvasItem *item)
{
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
 ...
 **/
void
mate_canvas_item_hide (MateCanvasItem *item)
{
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
 ...
 * the specified item was hidden by calling mate_canvas_item_hide(), then it
 * returns %GDK_GRAB_NOT_VIEWABLE.  Else, it returns the result of calling
 * gdk_pointer_grab().
 **/
int
mate_canvas_item_grab (MateCanvasItem *item, guint event_mask, GdkCursor *cursor, guint32 etime)
{
	int retval;

	g_return_val_if_fail (MATE_IS_CANVAS_ITEM (item), GDK_GRAB_NOT_VIEWABLE);
 ...
 **/
void
mate_canvas_item_ungrab (MateCanvasItem *item, guint32 etime)
{
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
 ...
 **/
void
mate_canvas_item_i2w_affine (MateCanvasItem *item, double affine[6])
{
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
	g_return_if_fail (affine != NULL);
 ...
 **/
void
mate_canvas_item_w2i (MateCanvasItem *item, double *x, double *y)
{
	...
	mate_canvas_item_i2w_affine (item, affine);
 ...
 **/
void
mate_canvas_item_i2w (MateCanvasItem *item, double *x, double *y)
{
	double affine[6];
	ArtPoint w, i;
 ...
 **/
void
mate_canvas_item_i2c_affine (MateCanvasItem *item, double affine[6])
{
	double i2w[6], w2c[6];
...
static int
is_descendant (MateCanvasItem *item, MateCanvasItem *parent)
{
 ...
 **/
void
mate_canvas_item_reparent (MateCanvasItem *item, MateCanvasGroup *new_group)
{
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
	g_return_if_fail (MATE_IS_CANVAS_GROUP (new_group));

	/* Both items need to be in the same canvas */
	g_return_if_fail (item->canvas == MATE_CANVAS_ITEM (new_group)->canvas);
	 ...
	 * the canvas.  */
	g_return_if_fail (!is_descendant (MATE_CANVAS_ITEM (new_group), item));
	...
	group_remove (MATE_CANVAS_GROUP (item->parent), item);
	item->parent = MATE_CANVAS_ITEM (new_group);
 ...
 **/
void
mate_canvas_item_grab_focus (MateCanvasItem *item)
{
 ...
 **/
void
mate_canvas_item_get_bounds (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	...
	if (MATE_CANVAS_ITEM_GET_CLASS (item)->bounds)
		(* MATE_CANVAS_ITEM_GET_CLASS (item)->bounds) (item, &tx1, &ty1, &tx2, &ty2);
...
/**
 * mate_canvas_item_request_update
 * @item: A canvas item.
 *
 ...
 **/
void
mate_canvas_item_request_update (MateCanvasItem *item)
{
	if (item->object.flags & MATE_CANVAS_ITEM_NEED_UPDATE)
		return;

	item->object.flags |= MATE_CANVAS_ITEM_NEED_UPDATE;

	if (item->parent != NULL) {
		/* Recurse up the tree */
		mate_canvas_item_request_update (item->parent);
	} else {
		/* Have reached the top of the tree, make sure the update call gets scheduled. */
		mate_canvas_request_update (item->canvas);
	}
}
...
static void mate_canvas_group_set_property(GObject               *object,
					    ...
					    const GValue          *value,
					    GParamSpec            *pspec);
static void mate_canvas_group_get_property(GObject               *object,
					    guint                  param_id,
					    GValue                *value,
					    GParamSpec            *pspec);
...
static void   mate_canvas_group_realize     (MateCanvasItem *item);
...
static void   mate_canvas_group_bounds      (MateCanvasItem *item, double *x1, double *y1,
					      double *x2, double *y2);
static void   mate_canvas_group_render      (MateCanvasItem *item,
					      MateCanvasBuf *buf);


static MateCanvasItemClass *group_parent_class;


/**
 * mate_canvas_group_get_type:
 *
 * Registers the &MateCanvasGroup class if necessary, and returns the type ID
 * associated to it.
 *
 * Return value:  The type ID of the &MateCanvasGroup class.
 **/
GType
mate_canvas_group_get_type (void)
{
	static GType canvas_group_type;

	if (!canvas_group_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasGroupClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_group_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasGroup),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_group_init,
			NULL			/* value_table */
		};

		canvas_group_type = g_type_register_static (MATE_TYPE_CANVAS_ITEM, "MateCanvasGroup",
							    &object_info, 0);
	}
...
/* Class initialization function for MateCanvasGroupClass */
static void
mate_canvas_group_class_init (MateCanvasGroupClass *class)
{
	...
	gobject_class->set_property = mate_canvas_group_set_property;
	gobject_class->get_property = mate_canvas_group_get_property;
	...
	object_class->destroy = mate_canvas_group_destroy;

	item_class->update = mate_canvas_group_update;
	item_class->realize = mate_canvas_group_realize;
	item_class->unrealize = mate_canvas_group_unrealize;
	item_class->map = mate_canvas_group_map;
	item_class->unmap = mate_canvas_group_unmap;
	item_class->draw = mate_canvas_group_draw;
	item_class->render = mate_canvas_group_render;
	item_class->point = mate_canvas_group_point;
	item_class->bounds = mate_canvas_group_bounds;
}

/* Object initialization function for MateCanvasGroup */
static void
mate_canvas_group_init (MateCanvasGroup *group)
{
#if 0
	group->xpos = 0.0;
	group->ypos = 0.0;
...
static double *
mate_canvas_ensure_translate (MateCanvasItem *item)
{
	if (item->xform == NULL) {
		GTK_OBJECT_UNSET_FLAGS (item, MATE_CANVAS_ITEM_AFFINE_FULL);
		item->xform = g_new (double, 2);
		item->xform[0] = 0.0;
		item->xform[1] = 0.0;
		return item->xform;
	} else if (item->object.flags & MATE_CANVAS_ITEM_AFFINE_FULL) {
		return item->xform + 4;
	} else {
		return item->xform;
	}
...
static void
mate_canvas_group_set_property (GObject *gobject, guint param_id,
				 const GValue *value, GParamSpec *pspec)
{
	MateCanvasItem *item;
	double *xlat;

	g_return_if_fail (MATE_IS_CANVAS_GROUP (gobject));

	item = MATE_CANVAS_ITEM (gobject);

	switch (param_id) {
	case GROUP_PROP_X:
		xlat = mate_canvas_ensure_translate (item);
...
static void
mate_canvas_group_get_property (GObject *gobject, guint param_id,
				 GValue *value, GParamSpec *pspec)
{
	MateCanvasItem *item;
	...
	case GROUP_PROP_X:
		...
		else if (GTK_OBJECT (gobject)->flags & MATE_CANVAS_ITEM_AFFINE_FULL)
...
#endif
{
	...
	g_return_if_fail (MATE_IS_CANVAS_GROUP (object));

	group = MATE_CANVAS_GROUP (object);
...
static void
mate_canvas_group_update (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *i;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		mate_canvas_item_invoke_update (i, affine, clip_path, flags);
...
static void
mate_canvas_group_realize (MateCanvasItem *item)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *i;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (!(i->object.flags & MATE_CANVAS_ITEM_REALIZED))
			(* MATE_CANVAS_ITEM_GET_CLASS (i)->realize) (i);
	}
...
static void
mate_canvas_group_unrealize (MateCanvasItem *item)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *i;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (i->object.flags & MATE_CANVAS_ITEM_REALIZED)
			(* MATE_CANVAS_ITEM_GET_CLASS (i)->unrealize) (i);
	}
...
static void
mate_canvas_group_map (MateCanvasItem *item)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *i;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (!(i->object.flags & MATE_CANVAS_ITEM_MAPPED))
			(* MATE_CANVAS_ITEM_GET_CLASS (i)->map) (i);
	}
...
static void
mate_canvas_group_unmap (MateCanvasItem *item)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *i;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (i->object.flags & MATE_CANVAS_ITEM_MAPPED)
			(* MATE_CANVAS_ITEM_GET_CLASS (i)->unmap) (i);
	}
...
static void
mate_canvas_group_draw (MateCanvasItem *item, GdkDrawable *drawable,
			 int x, int y, int width, int height)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *child = NULL;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (((child->object.flags & MATE_CANVAS_ITEM_VISIBLE)
		     && ((child->x1 < (x + width))
			 && (child->y1 < (y + height))
			 && (child->x2 > x)
			 && (child->y2 > y)))
		    || ((GTK_OBJECT_FLAGS (child) & MATE_CANVAS_ITEM_ALWAYS_REDRAW)
			...
			if (MATE_CANVAS_ITEM_GET_CLASS (child)->draw)
				(* MATE_CANVAS_ITEM_GET_CLASS (child)->draw) (
...
static double
mate_canvas_group_point (MateCanvasItem *item, double x, double y, int cx, int cy,
			  MateCanvasItem **actual_item)
{
	MateCanvasGroup *group;
	GList *list;
	MateCanvasItem *child, *point_item;
	int x1, y1, x2, y2;
	double gx, gy;
	double dist, best;
	int has_point;
	...
	*actual_item = NULL;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if ((child->object.flags & MATE_CANVAS_ITEM_VISIBLE)
		    && MATE_CANVAS_ITEM_GET_CLASS (child)->point) {
			dist = mate_canvas_item_invoke_point (child, gx, gy, cx, cy, &point_item);
			has_point = TRUE;
		} else
			has_point = FALSE;
...
static void
mate_canvas_group_bounds (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	MateCanvasGroup *group;
	MateCanvasItem *child;
	GList *list;
	double tx1, ty1, tx2, ty2;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (child->object.flags & MATE_CANVAS_ITEM_VISIBLE) {
			set = TRUE;
			mate_canvas_item_get_bounds (child, &minx, &miny, &maxx, &maxy);
			break;
		}
	}
	...
	for (; list; list = list->next) {
		...
		mate_canvas_item_get_bounds (child, &tx1, &ty1, &tx2, &ty2);
...
static void
mate_canvas_group_render (MateCanvasItem *item, MateCanvasBuf *buf)
{
	MateCanvasGroup *group;
	MateCanvasItem *child;
	GList *list;
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (((child->object.flags & MATE_CANVAS_ITEM_VISIBLE)
			...
			&& (child->x2 > child->canvas->redraw_x1)
			&& (child->y2 > child->canvas->redraw_y2)))
			if (MATE_CANVAS_ITEM_GET_CLASS (child)->render)
				(* MATE_CANVAS_ITEM_GET_CLASS (child)->render) (
					child, buf);
	}
}
...
static void
group_add (MateCanvasGroup *group, MateCanvasItem *item)
{
	...
	if (group->item.object.flags & MATE_CANVAS_ITEM_REALIZED)
		(* MATE_CANVAS_ITEM_GET_CLASS (item)->realize) (item);

	if (group->item.object.flags & MATE_CANVAS_ITEM_MAPPED)
		(* MATE_CANVAS_ITEM_GET_CLASS (item)->map) (item);
...
static void
group_remove (MateCanvasGroup *group, MateCanvasItem *item)
{
	GList *children;

	g_return_if_fail (MATE_IS_CANVAS_GROUP (group));
	g_return_if_fail (MATE_IS_CANVAS_ITEM (item));
...
static void mate_canvas_unrealize           (GtkWidget        *widget);
static void mate_canvas_size_allocate       (GtkWidget        *widget,
					      GtkAllocation    *allocation);
static gint mate_canvas_button              (GtkWidget        *widget,
					      GdkEventButton   *event);
static gint mate_canvas_motion              (GtkWidget        *widget,
					      GdkEventMotion   *event);
static gint mate_canvas_expose              (GtkWidget        *widget,
...
static gint mate_canvas_focus_in            (GtkWidget        *widget,
					      GdkEventFocus    *event);
static gint mate_canvas_focus_out           (GtkWidget        *widget,
					      GdkEventFocus    *event);
static void mate_canvas_request_update_real (MateCanvas      *canvas);
static void mate_canvas_draw_background     (MateCanvas      *canvas,
...
/**
 * mate_canvas_get_type:
 *
 * Registers the &MateCanvas class if necessary, and returns the type ID
 * associated to it.
 *
 * Return value:  The type ID of the &MateCanvas class.
 **/
GType
mate_canvas_get_type (void)
{
	static GType canvas_type;

	if (!canvas_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvas),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_init,
			NULL			/* value_table */
		};

		canvas_type = g_type_register_static (GTK_TYPE_LAYOUT, "MateCanvas",
						      &object_info, 0);
	}
			   ...
			   GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_AA:
		g_value_set_boolean (value, MATE_CANVAS (object)->aa);
		break;
	case PROP_FOCUSED_ITEM:
		g_value_set_object (value, MATE_CANVAS (object)->focused_item);
		break;
	default:
			   ...
			   GParamSpec   *pspec)
{
	switch (prop_id) {
	case PROP_AA:
		MATE_CANVAS (object)->aa = g_value_get_boolean (value);
		break;
	case PROP_FOCUSED_ITEM:
		MATE_CANVAS (object)->focused_item = g_value_get_object (value);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
		break;
...
/* Class initialization function for MateCanvasClass */
static void
mate_canvas_class_init (MateCanvasClass *klass)
{
	...
	gobject_class->set_property = mate_canvas_set_property;
	gobject_class->get_property = mate_canvas_get_property;

	object_class->destroy = mate_canvas_destroy;

	widget_class->map = mate_canvas_map;
	widget_class->unmap = mate_canvas_unmap;
	widget_class->realize = mate_canvas_realize;
	widget_class->unrealize = mate_canvas_unrealize;
	widget_class->size_allocate = mate_canvas_size_allocate;
	widget_class->button_press_event = mate_canvas_button;
	widget_class->button_release_event = mate_canvas_button;
	widget_class->motion_notify_event = mate_canvas_motion;
	widget_class->expose_event = mate_canvas_expose;
	widget_class->key_press_event = mate_canvas_key;
	widget_class->key_release_event = mate_canvas_key;
	widget_class->enter_notify_event = mate_canvas_crossing;
	...
	klass->request_update = mate_canvas_request_update_real;
	...
	canvas_signals[DRAW_BACKGROUND] =
		g_signal_new ("draw_background",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (MateCanvasClass, draw_background),
			      NULL, NULL,
			      mate_canvas_marshal_VOID__OBJECT_INT_INT_INT_INT,
	...
	canvas_signals[RENDER_BACKGROUND] =
		g_signal_new ("render_background",
			      G_TYPE_FROM_CLASS (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (MateCanvasClass, render_background),
			      NULL, NULL,
...
/* Object initialization function for MateCanvas */
static void
mate_canvas_init (MateCanvas *canvas)
{
	 ...
	 * default to have the same initial behavior as the canvas in MATE 1.4.
	 */
...
static void
shutdown_transients (MateCanvas *canvas)
{
...
#endif
{
	...
	g_return_if_fail (MATE_IS_CANVAS (object));

	/* remember, destroy can be run multiple times! */

	canvas = MATE_CANVAS (object);
 ...
 **/
GtkWidget *
mate_canvas_new (void)
{
	return GTK_WIDGET (g_object_new (mate_canvas_get_type (), NULL));
}
 ...
 **/
GtkWidget *
mate_canvas_new_aa (void)
{
	return GTK_WIDGET (g_object_new (MATE_TYPE_CANVAS,
					 "aa", TRUE,
					 NULL));
}
...
static void
mate_canvas_map (GtkWidget *widget)
{
	MateCanvas *canvas;

	g_return_if_fail (MATE_IS_CANVAS (widget));
	...
	if (MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->map)
		(* MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->map) (canvas->root);
}

/* Unmap handler for the canvas */
static void
mate_canvas_unmap (GtkWidget *widget)
{
	MateCanvas *canvas;
	...
	if (MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->unmap)
		(* MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->unmap) (canvas->root);
...
static void
mate_canvas_realize (GtkWidget *widget)
{
	MateCanvas *canvas;
	...
	(* MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->realize) (canvas->root);
}

/* Unrealize handler for the canvas */
static void
mate_canvas_unrealize (GtkWidget *widget)
{
	MateCanvas *canvas;
	...
	(* MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->unrealize) (canvas->root);
 ...
 */
static void
scroll_to (MateCanvas *canvas, int cx, int cy)
{
	...
	if ((canvas->zoom_xofs != old_zoom_xofs) || (canvas->zoom_yofs != old_zoom_yofs)) {
		...
		if (!(canvas->root->object.flags & MATE_CANVAS_ITEM_NEED_AFFINE)) {
			canvas->root->object.flags |= MATE_CANVAS_ITEM_NEED_AFFINE;
			mate_canvas_request_update (canvas);
		}
...
static void
mate_canvas_size_allocate (GtkWidget *widget, GtkAllocation *allocation)
{
	MateCanvas *canvas;
...
static int
emit_event (MateCanvas *canvas, GdkEvent *event)
{
	GdkEvent *ev;
	gint finished;
	MateCanvasItem *item;
...
/*                g_warning ("emit_event() returning FALSE!\n");*/
	...
	case GDK_ENTER_NOTIFY:
	case GDK_LEAVE_NOTIFY:
		mate_canvas_window_to_world (canvas,
 ...
 */
static int
pick_current_item (MateCanvas *canvas, GdkEvent *event)
{
	int button_down;
	double x, y;
	int cx, cy;
	...
	if (canvas->pick_event.type != GDK_LEAVE_NOTIFY) {
		...
		if (canvas->root->object.flags & MATE_CANVAS_ITEM_VISIBLE)
			mate_canvas_item_invoke_point (canvas->root, x, y, cx, cy,
...
static gint
mate_canvas_button (GtkWidget *widget, GdkEventButton *event)
{
	MateCanvas *canvas;
	int mask;
	int retval;
...
static gint
mate_canvas_motion (GtkWidget *widget, GdkEventMotion *event)
{
	MateCanvas *canvas;
...
static gint
mate_canvas_crossing (GtkWidget *widget, GdkEventCrossing *event)
{
	MateCanvas *canvas;
...
static gint
mate_canvas_focus_in (GtkWidget *widget, GdkEventFocus *event)
{
	MateCanvas *canvas;
...
static gint
mate_canvas_focus_out (GtkWidget *widget, GdkEventFocus *event)
{
	MateCanvas *canvas;
...
static void
mate_canvas_paint_rect (MateCanvas *canvas, gint x0, gint y0, gint x1, gint y1)
{
	GtkWidget *widget;
	...
	if (canvas->aa) {
		...
		if (canvas->root->object.flags & MATE_CANVAS_ITEM_VISIBLE)
			(* MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->render) (canvas->root, &buf);
		...
		if (canvas->root->object.flags & MATE_CANVAS_ITEM_VISIBLE)
			(* MATE_CANVAS_ITEM_GET_CLASS (canvas->root)->draw) (
				canvas->root, pixmap,
				draw_x1, draw_y1,
				draw_width, draw_height);
...
static gint
mate_canvas_expose (GtkWidget *widget, GdkEventExpose *event)
{
	MateCanvas *canvas;
	GdkRectangle *rects;
	gint n_rects;
	...
	for (i = 0; i < n_rects; i++) {
		...
		if (canvas->need_update || canvas->need_redraw) {
			...
			mate_canvas_request_redraw_uta (canvas, uta);
		} else {
			/* No pending updates, draw exposed area immediately */
			mate_canvas_paint_rect (canvas, rect.x0, rect.y0, rect.x1, rect.y1);
...
static void
mate_canvas_draw_background (MateCanvas *canvas, GdkDrawable *drawable,
			      int x, int y, int width, int height)
{
...
update_again:
	if (canvas->need_update) {
		...
		mate_canvas_item_invoke_update (canvas->root, w2cpx, NULL, 0);
 ...
 **/
MateCanvasGroup *
mate_canvas_root (MateCanvas *canvas)
{
	g_return_val_if_fail (MATE_IS_CANVAS (canvas), NULL);

	return MATE_CANVAS_GROUP (canvas->root);
}


/**
 * mate_canvas_set_scroll_region:
 * @canvas: A canvas.
 ...
 **/
void
mate_canvas_set_scroll_region (MateCanvas *canvas, double x1, double y1, double x2, double y2)
{
	double wxofs, wyofs;
	int xofs, yofs;

	g_return_if_fail (MATE_IS_CANVAS (canvas));
	...
	mate_canvas_w2c (canvas, wxofs, wyofs, &xofs, &yofs);
	...
	(* MATE_CANVAS_ITEM_CLASS (canvas->root->object.klass)->update) (
 ...
 **/
void
mate_canvas_get_scroll_region (MateCanvas *canvas, double *x1, double *y1, double *x2, double *y2)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
...
/**
 * mate_canvas_set_center_scroll_region:
 * @canvas: A canvas.
 ...
 **/
void
mate_canvas_set_center_scroll_region (MateCanvas *canvas, gboolean center_scroll_region)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
 ...
 **/
gboolean
mate_canvas_get_center_scroll_region (MateCanvas *canvas)
{
	g_return_val_if_fail (MATE_IS_CANVAS (canvas), FALSE);

	return canvas->center_scroll_region ? TRUE : FALSE;
}

/**
 * mate_canvas_set_pixels_per_unit:
 * @canvas: A canvas.
 ...
 **/
void
mate_canvas_set_pixels_per_unit (MateCanvas *canvas, double n)
{
	double ax, ay;
	int x1, y1;
	...
	g_return_if_fail (MATE_IS_CANVAS (canvas));
	g_return_if_fail (n > MATE_CANVAS_EPSILON);
 ...
 **/
void
mate_canvas_scroll_to (MateCanvas *canvas, int cx, int cy)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
...
/**
 * mate_canvas_get_scroll_offsets:
 * @canvas: A canvas.
 ...
 **/
void
mate_canvas_get_scroll_offsets (MateCanvas *canvas, int *cx, int *cy)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
 ...
 */
void
mate_canvas_update_now (MateCanvas *canvas)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
 ...
 **/
MateCanvasItem *
mate_canvas_get_item_at (MateCanvas *canvas, double x, double y)
{
	MateCanvasItem *item;
	double dist;
	int cx, cy;
	...
	mate_canvas_w2c (canvas, x, y, &cx, &cy);

	dist = mate_canvas_item_invoke_point (canvas->root, x, y, cx, cy, &item);
...
static void
mate_canvas_request_update (MateCanvas *canvas)
{
	MATE_CANVAS_GET_CLASS (canvas)->request_update (canvas);
}

static void
mate_canvas_request_update_real (MateCanvas *canvas)
{
	if (canvas->need_update)
		return;
...
static inline void
get_visible_region (MateCanvas *canvas, ArtIRect *visible)
{
...
/**
 * mate_canvas_request_redraw_uta:
 * @canvas: A canvas.
 ...
 **/
void
mate_canvas_request_redraw_uta (MateCanvas *canvas,
                                 ArtUta      *uta)
{
	ArtIRect visible;
 ...
 **/
void
mate_canvas_request_redraw (MateCanvas *canvas, int x1, int y1, int x2, int y2)
{
	ArtUta *uta;
	ArtIRect bbox;
	ArtIRect visible;
 ...
 **/
void
mate_canvas_w2c_affine (MateCanvas *canvas, double affine[6])
{
	double zooom;
 ...
 **/
void
mate_canvas_w2c (MateCanvas *canvas, double wx, double wy, int *cx, int *cy)
{
	double affine[6];
	ArtPoint w, c;
	...
	mate_canvas_w2c_affine (canvas, affine);
	w.x = wx;
	w.y = wy;
 ...
 **/
void
mate_canvas_w2c_d (MateCanvas *canvas, double wx, double wy, double *cx, double *cy)
{
	double affine[6];
	ArtPoint w, c;
 ...
 **/
void
mate_canvas_c2w (MateCanvas *canvas, int cx, int cy, double *wx, double *wy)
{
	double affine[6], inv[6];
	ArtPoint w, c;
 ...
 **/
void
mate_canvas_window_to_world (MateCanvas *canvas, double winx, double winy,
 ...
 **/
void
mate_canvas_world_to_window (MateCanvas *canvas, double worldx, double worldy,
 ...
 **/
int
mate_canvas_get_color (MateCanvas *canvas, const char *spec, GdkColor *color)
{
 ...
 **/
gulong
mate_canvas_get_color_pixel (MateCanvas *canvas, guint rgba)
{
	GdkColormap *colormap;
	GdkColor color;

	g_return_val_if_fail (MATE_IS_CANVAS (canvas), 0);
...
/**
 * mate_canvas_set_stipple_origin:
 * @canvas: A canvas.
 ...
 **/
void
mate_canvas_set_stipple_origin (MateCanvas *canvas, GdkGC *gc)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
 ...
 **/
void
mate_canvas_set_dither (MateCanvas *canvas, GdkRgbDither dither)
{
	g_return_if_fail (MATE_IS_CANVAS (canvas));
 ...
 **/
GdkRgbDither
mate_canvas_get_dither (MateCanvas *canvas)
{
	g_return_val_if_fail (MATE_IS_CANVAS (canvas), GDK_RGB_DITHER_NONE);
...
/* Class initialization function for MateCanvasItemClass */
static void
mate_canvas_item_class_init (MateCanvasItemClass *class)
{
	GObjectClass *gobject_class;
	...
	gobject_class->set_property = mate_canvas_item_set_property;
	gobject_class->get_property = mate_canvas_item_get_property;
	...
	item_signals[ITEM_EVENT] =
		g_signal_new ("event",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (MateCanvasItemClass, event),
			      boolean_handled_accumulator, NULL,
			      mate_canvas_marshal_BOOLEAN__BOXED,
			      G_TYPE_BOOLEAN, 1,
			      GDK_TYPE_EVENT | G_SIGNAL_TYPE_STATIC_SCOPE);

	gobject_class->dispose = mate_canvas_item_dispose;

	class->realize = mate_canvas_item_realize;
	class->unrealize = mate_canvas_item_unrealize;
	class->map = mate_canvas_item_map;
	class->unmap = mate_canvas_item_unmap;
	class->update = mate_canvas_item_update;
}
