...
#include "mate-canvas-pixbuf.h"

/* Private part of the MateCanvasPixbuf structure */
typedef struct {
	/* Our gdk-pixbuf */
	GdkPixbuf *pixbuf;
...
static void mate_canvas_pixbuf_class_init (MateCanvasPixbufClass *class);
static void mate_canvas_pixbuf_init (MateCanvasPixbuf *cpb);
static void mate_canvas_pixbuf_destroy (GtkObject *object);
static void mate_canvas_pixbuf_set_property (GObject *object,
					      guint param_id,
					      const GValue *value,
					      GParamSpec *pspec);
static void mate_canvas_pixbuf_get_property (GObject *object,
					      guint param_id,
					      GValue *value,
					      GParamSpec *pspec);

static void mate_canvas_pixbuf_update (MateCanvasItem *item, double *affine,
					ArtSVP *clip_path, int flags);
static void mate_canvas_pixbuf_draw (MateCanvasItem *item, GdkDrawable *drawable,
				      int x, int y, int width, int height);
static void mate_canvas_pixbuf_render (MateCanvasItem *item, MateCanvasBuf *buf);
static double mate_canvas_pixbuf_point (MateCanvasItem *item, double x, double y, int cx, int cy,
					 MateCanvasItem **actual_item);
static void mate_canvas_pixbuf_bounds (MateCanvasItem *item,
...
/**
 * mate_canvas_pixbuf_get_type:
 * @void:
 *
 * Registers the #MateCanvasPixbuf class if necessary, and returns the type ID
 * associated to it.
 *
 * Return value: The type ID of the #MateCanvasPixbuf class.
 **/
GType
mate_canvas_pixbuf_get_type (void)
{
	static GType pixbuf_type;

	if (!pixbuf_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasPixbufClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_pixbuf_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasPixbuf),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_pixbuf_init,
			NULL			/* value_table */
		};

		pixbuf_type = g_type_register_static (MATE_TYPE_CANVAS_ITEM, "MateCanvasPixbuf",
						     &object_info, 0);
	}
...
static void
mate_canvas_pixbuf_class_init (MateCanvasPixbufClass *class)
{
	...
	gobject_class->set_property = mate_canvas_pixbuf_set_property;
	gobject_class->get_property = mate_canvas_pixbuf_get_property;
	...
	object_class->destroy = mate_canvas_pixbuf_destroy;

	item_class->update = mate_canvas_pixbuf_update;
	item_class->draw = mate_canvas_pixbuf_draw;
	item_class->render = mate_canvas_pixbuf_render;
	item_class->point = mate_canvas_pixbuf_point;
	item_class->bounds = mate_canvas_pixbuf_bounds;
}

/* Object initialization function for the pixbuf canvas item */
static void
mate_canvas_pixbuf_init (MateCanvasPixbuf *gcp)
{
	PixbufPrivate *priv;
...
static void
mate_canvas_pixbuf_destroy (GtkObject *object)
{
	MateCanvasItem *item;
	MateCanvasPixbuf *gcp;
	PixbufPrivate *priv;

	g_return_if_fail (object != NULL);
	g_return_if_fail (MATE_IS_CANVAS_PIXBUF (object));

	item = MATE_CANVAS_ITEM (object);
	gcp = (MATE_CANVAS_PIXBUF (object));
	priv = gcp->priv;
...
static void
mate_canvas_pixbuf_set_property (GObject            *object,
				  guint               param_id,
				  const GValue       *value,
				  GParamSpec         *pspec)
{
...
static void
mate_canvas_pixbuf_get_property (GObject            *object,
				  guint               param_id,
				  GValue             *value,
				  GParamSpec         *pspec)
{
 ...
 */
static void
compute_viewport_affine (MateCanvasPixbuf *gcp, double *viewport_affine, double *i2c)
{
	PixbufPrivate *priv;
	ArtPoint i_c, j_c;
	double i_len, j_len;
 ...
 */
static void
compute_render_affine (MateCanvasPixbuf *gcp, double *ra, double *i2c)
{
	double va[6];

	compute_viewport_affine (gcp, va, i2c);
#ifdef MATE_CANVAS_PIXBUF_VERBOSE
 ...
 */
static void
recompute_bounding_box (MateCanvasPixbuf *gcp, gdouble *i2c)
{
	MateCanvasItem *item;
	PixbufPrivate *priv;
	double ra[6];
...
static void
mate_canvas_pixbuf_update (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	MateCanvasPixbuf *gcp;
	PixbufPrivate *priv;

	gcp = MATE_CANVAS_PIXBUF (item);
	priv = gcp->priv;
	 ...
	 * throughgout all of mate-canvas-pixbuf.c - TIMJ
	 */
#ifdef USE_BROKEN_OPTIMIZATIONS
	if (((flags & MATE_CANVAS_UPDATE_VISIBILITY)
	     && !(GTK_OBJECT_FLAGS (item) & MATE_CANVAS_ITEM_VISIBLE))
	...
	if (priv->need_pixbuf_update
	    || priv->need_xform_update
	    || ((flags & MATE_CANVAS_UPDATE_VISIBILITY)
		&& (GTK_OBJECT_FLAGS (gcp) & MATE_CANVAS_ITEM_VISIBLE))
	    || (flags & MATE_CANVAS_UPDATE_AFFINE)) {
...
static void
mate_canvas_pixbuf_draw (MateCanvasItem *item, GdkDrawable *drawable,
			  int x, int y, int width, int height)
{
	MateCanvasPixbuf *gcp;
	PixbufPrivate *priv;
...
static void
mate_canvas_pixbuf_render (MateCanvasItem *item, MateCanvasBuf *buf)
{
	MateCanvasPixbuf *gcp;
	PixbufPrivate *priv;
	double i2c[6], render_affine[6];
	...
	if ((fabs (render_affine[1]) < MATE_CANVAS_EPSILON) &&
	    (fabs (render_affine[2]) < MATE_CANVAS_EPSILON) &&
	    render_affine[0] > 0.0 &&
	    render_affine[3] > 0.0)
	  {
...
static double
mate_canvas_pixbuf_point (MateCanvasItem *item, double x, double y, int cx, int cy,
			   MateCanvasItem **actual_item)
{
	MateCanvasPixbuf *gcp;
	PixbufPrivate *priv;
...
static void
mate_canvas_pixbuf_bounds (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	MateCanvasPixbuf *gcp;
	PixbufPrivate *priv;
