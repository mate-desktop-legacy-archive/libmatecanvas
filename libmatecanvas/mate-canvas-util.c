 ...
 **/
MateCanvasPoints *
mate_canvas_points_new (int num_points)
{
	MateCanvasPoints *points;

	g_return_val_if_fail (num_points > 1, NULL);

	points = g_new (MateCanvasPoints, 1);
 ...
 **/
MateCanvasPoints *
mate_canvas_points_ref (MateCanvasPoints *points)
{
 ...
 **/
void
mate_canvas_points_free (MateCanvasPoints *points)
{
 ...
 **/
int
mate_canvas_get_miter_points (double x1, double y1, double x2, double y2, double x3, double y3,
			       double width,
 ...
 **/
void
mate_canvas_get_butt_points (double x1, double y1, double x2, double y2,
 ...
 **/
double
mate_canvas_polygon_to_point (double *poly, int num_points, double x, double y)
{
	double best;
	int intersections;
	int i;
 ...
 **/
void
mate_canvas_render_svp (MateCanvasBuf *buf, ArtSVP *svp, guint32 rgba)
{
 ...
 **/
void
mate_canvas_update_svp (MateCanvas *canvas, ArtSVP **p_svp, ArtSVP *new_svp)
{
	ArtSVP *old_svp;
	...
	if (old_svp != NULL) {
		...
		if ((bb.x1 - bb.x0) * (bb.y1 - bb.y0) > (64 * 64)) {
			...
			mate_canvas_request_redraw_uta (canvas, repaint_uta);
		} else {
			ArtIRect ib;
			art_drect_to_irect (&ib, &bb);
			mate_canvas_request_redraw (canvas, ib.x0, ib.y0, ib.x1, ib.y1);
		}
 ...
 **/
void
mate_canvas_update_svp_clip (MateCanvas *canvas, ArtSVP **p_svp, ArtSVP *new_svp, ArtSVP *clip_svp)
{
	ArtSVP *clipped_svp;
	...
	mate_canvas_update_svp (canvas, p_svp, clipped_svp);
}
 ...
 **/
void
mate_canvas_item_reset_bounds (MateCanvasItem *item)
{
	item->x1 = 0.0;
	item->y1 = 0.0;
	item->x2 = 0.0;
 ...
 **/
void
mate_canvas_item_update_svp (MateCanvasItem *item, ArtSVP **p_svp, ArtSVP *new_svp)
{
	ArtDRect bbox;

	mate_canvas_update_svp (item->canvas, p_svp, new_svp);
	if (new_svp) {
		bbox.x0 = item->x1;
		bbox.y0 = item->y1;
		bbox.x1 = item->x2;
 ...
 **/
void
mate_canvas_item_update_svp_clip (MateCanvasItem *item, ArtSVP **p_svp, ArtSVP *new_svp,
				   ArtSVP *clip_svp)
{
	ArtSVP *clipped_svp;
	...
	mate_canvas_item_update_svp (item, p_svp, clipped_svp);
}
 ...
 **/ 
void
mate_canvas_item_request_redraw_svp (MateCanvasItem *item, const ArtSVP *svp)
{
	MateCanvas *canvas;
	ArtUta *uta;
	...
	if (canvas->aa) {
		...
		mate_canvas_request_redraw (canvas, item->x1, item->y1, item->x2, item->y2);		
	}
}
 ...
 **/
void
mate_canvas_update_bbox (MateCanvasItem *item, int x1, int y1, int x2, int y2)
{
 ...
 **/
void
mate_canvas_buf_ensure_buf (MateCanvasBuf *buf)
{
	guchar *bufptr;
	int y;
...
mate_canvas_join_gdk_to_art (GdkJoinStyle gdk_join)
{
	switch (gdk_join) {
	case GDK_JOIN_MITER:
...
mate_canvas_cap_gdk_to_art (GdkCapStyle gdk_cap)
{
	switch (gdk_cap) {
	case GDK_CAP_BUTT:
