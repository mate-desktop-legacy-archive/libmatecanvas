...
static gboolean item_event   (MateCanvasItem *item,
			      GdkEvent        *event,
			      gpointer         dummy);

static gboolean canvas_event (MateCanvasItem *item,
			      GdkEvent        *event,
			      gpointer         dummy);
...
static void
draw_curve (MateCanvasItem *item,
	    gdouble          x,
	    gdouble          y)
{
	...
	case STATE_INIT: 
		...
		if (!current_points)
			current_points = mate_canvas_points_new (4);
	...
	case STATE_FIRST_PRESS:
		...
		if (current_item)
			...
			current_item = mate_canvas_item_new (
						root,
						mate_canvas_bpath_get_type(),
						"bpath", path_def,
						"outline_color", "blue",
						"width_pixels", 5,
...
static gboolean
item_event (MateCanvasItem *item,
	    GdkEvent        *event,
	    gpointer         dummy)
{
...
static gboolean
canvas_event (MateCanvasItem *item,
	      GdkEvent        *event,
	      gpointer         dummy)
{
	switch (event->type) {
...
create_canvas (gboolean aa)
{
	...
	mate_canvas_set_scroll_region (MATE_CANVAS (canvas), 0, 0, 600, 250);
	gtk_widget_show (canvas);

	root = mate_canvas_root (MATE_CANVAS (canvas));

	item = mate_canvas_item_new (root,
